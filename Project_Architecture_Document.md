**Last Updated:** 2026-09-23 (v1.26 — the ninth zero-drift verification round 27: the standard live re-probe with the two operator focus areas re-verified — the mobile navigation menu and the TailwindCSS v4 audit — found the live UNCHANGED STRUCTURALLY on every probed layer for the NINTH consecutive round (19/20/21/22/23/24/25/26/27): the body-DOM signature diff (the canonical plain tag.class signature) carries zero non-bucketed deltas on all 10 surfaces, with the single live delta a USER-DATA change (the user deleted their last income source — the Income view now renders its empty state, a −27/+9 multiset delta on that surface alone; the other eight desktop surfaces byte-identical to round-26's captures, and the clone multisets byte-identical on all 9), the mobile drawer is structurally byte-identical (71/71 nodes, only the lucide element-mix) and verified working end-to-end on both sides — open, navigate + unmount, Menu↔X swap both directions, X close, zero console messages on a clean cycle, the Tailwind v4 audit is clean (the 176,431-byte production bundle carries every drawer utility + all four fin-* keyframes declared inside @media (prefers-reduced-motion:no-preference), no dynamic class construction, and the v3 palette pins verified at the variable level — the utilities resolve through the variables and the unlayered :root pin wins the cascade at runtime, computed #2563eb/#1e293b/#ef4444/#34d399/#4b5563 on the CSS variables themselves, with consumer-element colors byte-matching the live: the emerald logo rgb(52, 211, 153) on both sides), the raw-order chrome probes are byte-identical (the aside/nav/innerPane strings and the active/inactive nav pills), the HTTP layer matches every round-17→26 pin byte-exact (the security trio + hardening pair, the per-route description template verified in-browser on the live + static on the clone — plus the root-route default description re-verified — the login-only pins on the live's static login HTML (two rendering content-first attribute order, values byte-matching), the per-route twitter:url + canonical, the SPA-style HTTP 200 for unknown paths), and the focused interactive sweep (FAB, chooser, step-2, the FAB close-toggle) matches on both sides — so the round is a verification round: the production-readiness review found the code side clean (gates green, the audit tail matching §10's documentation exactly at 43 advisories / ZERO in the runtime set, CI verified two ways — the marker-safe needles plus the od -c hexdump ground truth, with the display artifact this round even eating the `[m` sequence through cat -A output — od's space-separated bytes are the only display-safe ground truth for bracket-bearing needles — the installed versions matching every doc claim, the round-24/25/26 doc fixes grep-verified holding, no TODO/FIXME), one genuine doc-drift fix (the SKILL's stale 'has run 21 times' re-probe-method count → 27), and the deliverables are the round-27 plan + the refreshed screenshots + this documentation set, with no code changes; the live account's theme state left as found (dark — the probe's own Menu-locator mis-click hit the adjacent sun toggle mid-probe, detected via the consumer-color gradient probe, restored through the user dropdown, and re-verified html.dark at session close). Previous: v1.25 — the eighth zero-drift verification round 26:Previous: v1.24 — the seventh zero-drift verification round 25: the standard live re-probe with the two operator focus areas re-verified — the mobile navigation menu and the TailwindCSS v4 audit — found the live UNCHANGED on every probed layer for the SEVENTH consecutive round (19/20/21/22/23/24/25): the body-DOM signature diff carries zero non-bucketed deltas on all 10 surfaces (the live user's dismissed-insights empty state, seed-data counts — income 4v1, expenses 100v1, accounts 4v1, investments 8v1, goals 3v1 — chart SVG data, the lucide element-mix, the duplicated toaster, Next infra incl. the font-loader divs, all classified), the mobile drawer is structurally byte-identical (71/71 nodes, only the lucide element-mix) and verified working end-to-end on both sides — open, navigate + unmount, Menu↔X swap both directions, X close, zero new console errors, the Tailwind v4 audit is clean (the 176 KB production bundle carries every drawer utility + keyframe with the reduced-motion gate, no dynamic class construction, and the unlayered v3 palette pin wins the cascade at runtime — computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables themselves), the raw-order chrome probes are byte-identical (the aside/nav/innerPane strings and the active/inactive nav pills), the HTTP layer matches every round-17→24 pin byte-exact (the security trio + hardening pair, the per-route description template verified in-browser on the live, the login-only pins, the per-route twitter:url + canonical, the SPA-style HTTP 200 for unknown paths), and the focused interactive sweep (FAB, chooser, step-2, the FAB close-toggle) matches on both sides — so the round is a verification round: the production-readiness review found the code side clean (gates green on the fresh environment, the audit tail matching §10's documentation exactly at 43 advisories / ZERO in the runtime set, CI bytes marker-safe-verified clean, the seven round-24 doc fixes grep-verified holding, no TODO/FIXME), and the deliverables are the round-25 plan + the refreshed screenshots + this documentation set, with no code changes; the live user's theme state left as found (dark, verified before and after). Previous: v1.23 — the sixth zero-drift verification round 24 + documentation-debt alignment: the standard live re-probe with the two operator focus areas re-verified — the mobile navigation menu and the TailwindCSS v4 audit — found the live UNCHANGED on every probed layer for the SIXTH consecutive round (19/20/21/22/23/24): the body-DOM signature diff carries zero non-bucketed deltas on all 10 surfaces (the live user's dismissed-insights empty state, seed-data counts — income 4v1, expenses 100v1, accounts 4v1, investments 8v1, goals 3v1 — chart SVG data, theme-state icons, the duplicated toaster, Next infra incl. the font-loader divs, all byte-verified), the mobile drawer is structurally byte-identical (70/70 nodes, identical classed-element sequence) and verified working end-to-end on both sides against the production build — open, navigate + unmount, Menu↔X swap both directions, zero new console errors, the Tailwind v4 audit is clean (the 176 KB production bundle carries every drawer utility + keyframe with the reduced-motion gate, no dynamic class construction, and the unlayered v3 palette pin wins the cascade at runtime — computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables themselves, with consumer-element colors byte-matching the live: the emerald logo `rgb(52, 211, 153)` on both sides), the HTTP layer matches every round-17→23 pin byte-exact (the security trio + hardening pair, the per-route description template on all three metas, the login-only pins, the per-route twitter:url + canonical, the SPA-style HTTP 200 for unknown paths), and the focused interactive sweep (FAB, chooser, step-2, the FAB close-toggle) matches on both sides — so the round's remediation surface was the documentation debt the production-readiness review surfaced: seven doc-drift fixes (the push-runbook's wrong remote `task-management.git` → `financial-dashboard.git` + its stale no-CI claim, the 7→8 Prisma model count + wrong model names in README/SKILL/PAD, this PAD's stale 16.1.3 Next.js version + the tailwind.config.ts directory-tree entry + the 378 CI test count + the glossary's quarterly wording, and the SKILL Appendix B header) — no code changes; the audit tail matches §10 exactly (43 advisories, ZERO in the runtime set), CI bytes marker-safe-verified clean with the ANSI artifact re-demonstrated inside the round's own verification-script output labels, and the live user's theme state left as found (dark). Previous: v1.22 — the fifth zero-drift verification round 23: the standard live re-probe with the two operator focus areas re-verified — the mobile navigation menu and the TailwindCSS v4 audit — found the live UNCHANGED on every probed layer for the FIFTH consecutive round (19/20/21/22/23): the body-DOM signature diff carries zero non-bucketed deltas on all 10 surfaces, the desktop chrome and mobile drawer are byte-identical (the drawer verified working end-to-end on both sides in dev AND production — open, navigate, X-swap, zero new console errors), the Tailwind v4 audit is clean (the production bundle carries every drawer utility + keyframe, no dynamic class construction, and the unlayered v3 palette pin wins the cascade at runtime — computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables themselves), the HTTP layer matches every prior pin byte-exact, and the focused interactive sweep (FAB, chooser, step-2, the FAB close-toggle) matches on both sides — so the round is a verification round: the production-readiness review found nothing to remediate (gates green, the audit tail matching §10's documentation exactly — 43 advisories, ZERO in the runtime set — CI bytes marker-safe-verified clean), and the deliverables are the refreshed screenshots + this documentation set, with no code changes; the round extended the ANSI-artifact lesson — the `[m` reset eats tokens in ANY output stream including an agent's own printed labels, so assert on the bytes with marker-safe needle construction. Previous: v1.21 — the full-stack verification round 22: the standard live re-probe with the two operator focus areas — the mobile navigation menu and the TailwindCSS v4 audit — found the live UNCHANGED on every probed layer for the FOURTH consecutive round (19/20/21/22): the body-DOM signature diff carries zero non-bucketed deltas on all 10 surfaces, the desktop chrome and mobile drawer are byte-identical (the drawer verified working end-to-end on both sides in dev AND production — open, navigate, X-swap, zero console errors), the Tailwind v4 audit is clean (the production bundle carries every drawer utility + keyframe, no dynamic class construction, and the unlayered v3 palette pin wins the cascade at runtime — computed `#2563eb`/`#1e293b` on the CSS variables themselves, overriding v4.1.18's regenerated ramp and its `@supports lab()` re-definition), the HTTP layer matches every prior pin byte-exact, and the full interactive sweep (FAB, Quick Add both steps, Add Expense modal, AI Coach, Analytics chart configuration, theme toggle, a live expense CRUD round-trip) matches — so the round is a verification round: the production-readiness review found nothing to remediate (gates green on a fresh environment, the audit tail matching §10's documentation exactly, CI bytes hex-verified clean), and the deliverables are the refreshed screenshots + this documentation set, with no code changes. Previous: v1.20 — the database-path contract + suite-verification round 21: the standard live re-probe with a focused mobile-navigation audit found the live UNCHANGED on every probed layer — the dashboard body-DOM diff carries zero non-bucketed deltas (the live user has dismissed all AI-insight records, so the live renders the round-13 empty state; activity/budget row counts and chart/icon inner-stroke populations differ with the data), the mobile drawer is byte-identical on both sides and verified working in dev AND production (the round's explicit focus — open, navigate, X-swap, zero console errors; the Tailwind v4 audit alongside it is clean: the production bundle carries every drawer utility + the `fin-drawer-in` keyframes, no dynamic class construction exists, and the unlayered v3 palette pin beats Tailwind v4.1.18's regenerated ramp and its `@supports lab()` re-definition), and the HTTP layer matches every round-17/18/19/20 pin byte-exact — so the round closed the environment/tooling gaps the operator prompt pins: the `.env.example`'s documented-but-unimplemented database-path contract now exists (`src/lib/db-path.ts` — relative `file:` URLs anchor at prisma/schema.prisma CLI-equivalently, CWD-independent via the walk-up from the process CWD; absolute URLs (the E2E hermetic db/e2e.db) and PostgreSQL URLs pass through; a missing DATABASE_URL throws the crisp setup error — wired into `db.ts` through Prisma 6's `datasourceUrl`, pinned by `src/lib/__tests__/db-path.test.ts`), the `.env.example` drift is aligned (the phantom `db:seed` script / `docs/DEPLOYMENT.md §4` / `tests/db-path.test.ts` references replaced with the real seed flow, the README pointer, and the repo-convention test path), and the suite baselines are verified on the fresh environment (unit 434/434 — 428 + 6 net-new; E2E 67/67 after the chromium headless-shell rebuild, the round-19 environment-lesson class; lint 0; tsc 0; build clean). The round also ships the distilled `financial-dashboard_SKILL.md` (the to-distill-project-into-skill meta-skill applied to this codebase) and refreshed dev-server screenshots.)
...
|-------|-----------|---------|---------------|
| Web framework | Next.js (App Router, Turbopack) | 16.3.5 | Route handlers + RSC-capable shell; the reference app it clones is a React SPA, and Next's single-page mode fits both the clone and the deployment sandbox |
...
Layer 5: Persistence        — Prisma + SQLite schema (8 models, integer minor units).
...
├── prisma/schema.prisma                ← 8 models, Int *Minor money columns
...
└── (eslint.config.mjs, tsconfig.json, postcss.config.mjs, components.json)
```
...
erDiagram
    ...
    Setting {
        string key PK "unique — doubles as the seed lock + the insights-generation marker"
        string value
    }
    Insight {
        int id PK
        string insightType "anomaly|alert|trend|opportunity|prediction"
        string title
        string description
        string suggestedAction
        float confidenceScore "0..1"
        string category
        boolean isDismissed "dismiss = PATCH, record stays"
        datetime createdDate
    }
```

Notes: `Investment.shares` is a `Float` because share counts are not money (fractional shares are legitimate); prices and all other amounts are integers. `Setting.key` uniqueness is load-bearing — it implements the seed mutex (ADR-005) and the insights-generation marker (round 13). The `Insight` model is the round-13 persisted AI-insight record (the live's `TransactionInsight` equivalent).
...
- **Migrations (round 15):** a committed baseline — `prisma/migrations/20260919000000_init/migration.sql` (generated via `prisma migrate diff --from-empty` + `migration_lock.toml`, sqlite). Workflow split: dev and E2E keep `bun run db:push` (disposable DBs; the Playwright webServer reset depends on it); production deploys run `bun run db:deploy` (`prisma migrate deploy`) so schema evolution is reviewable SQL instead of data-loss sync. `src/lib/__tests__/migrations-baseline.test.ts` is the drift guard — every schema model must have its CREATE TABLE in a committed migration. The §10 item is Resolved.
...
The entrance-motion system is CSS-replicated from the live's framer-motion layer (round 14, correcting round 13's misdiagnosis — the live's entrances animate INLINE STYLES on wrapper divs, which never set CSS `animationName`; only the CSS-class animations were removed in the round-13 window). `entranceStyle(delayMs)` in `ui-bits.tsx` renders the settled inline style (`opacity: 1; transform: "none"`) plus an inline `animation` referencing the `fin-card-in` keyframes in globals.css — hand-fit to the rAF-sampled live curve (opacity 0→1 + translateY 20px→0, ~330ms, damped-spring −2.3px overshoot, ~100ms stagger per view in DOM order). Classless `MotionWrap` covers the cards the live wraps outside their roots; styled surfaces (header rows via `ViewHeader`, Dashboard tiles, list rows, the Quick Actions panel, the Analytics KPI grid) carry the style directly. The keyframes exist only under `@media (prefers-reduced-motion: no-preference)` — a missing keyframe list makes the inline animation a no-op, so reduced-motion users render the settled state instantly. List-row staggers are capped (`min(index, 8)×50ms` — a documented adaptation: the live's uncapped stagger would take 9.8s on the 98-row seed; the live never exercises >2 rows). The AI-insight row keeps its own `insight-row` entrance (round-13); the refresh-cw icons spin via the `animate-spin` class inside `transform: none` wrappers (CSS animations override the inline transform during playback — visually identical to the live's JS rotation; documented mechanism delta); the Quick Add chooser is the only dialog that animates open (`fin-overlay-in` overlay fade + `fin-scale-in` wrapper 0.9→1 spring, ~320ms — the full modals open instantly with settled `overlayStyle`s). Card hover keeps `.card-hover` (translate + shadow lift); chart mount animations from Recharts defaults.
...
| Security headers on every response (round 17) | `next.config.ts` `headers()` on `/:path*`: Referrer-Policy strict-origin-when-cross-origin, X-Content-Type-Options nosniff, Strict-Transport-Security max-age=31536000 (the live app's edge trio — live-probed) + X-Frame-Options DENY, Permissions-Policy camera=(), microphone=(), geolocation=() (hardening additions); `poweredByHeader: false` — pinned by `src/lib/__tests__/site-metadata.test.ts`; CSP is documented future work (a nonce strategy for Next's inline scripts) |

### 6.2 Security Utilities

`src/lib/api.ts` (guards + envelope), the `next.config.ts` security-header baseline (§6.1), `.gitignore` key rejection, `docs/ssh_git_wrapper_v3.py` (0600 temp key, shred-after-push, `IdentitiesOnly`).
...
| Unit suite | 434 tests / 18 files | Vitest 5 (node env) | `src/lib/__tests__/` |
| E2E suite | 67 specs / 11 files | Playwright 1.62 (chromium, serial) | `e2e/` — production build on :3100, hermetic `db/e2e.db` + `FINARA_INSIGHTS_LLM_OFF=1` (deterministic insight records) |
...
| `FINARA_INSIGHTS_LLM_OFF` | No | `1` skips the optional LLM polish of AI-insight records (deterministic drafts — hermetic gates; the E2E webServer sets it) | unset |
| `NEXT_PUBLIC_SITE_URL` | No | The deployable site origin (round 17) — resolves `metadataBase`, og:url/canonical links, and robots.txt/sitemap.xml URLs | `http://localhost:3000` |

AI access is configured by the hosting environment's SDK credentials (the `z-ai-web-dev-sdk` reads its own runtime configuration — nothing is committed).
...
| MEDIUM | No migration history (schema via `db:push`) | Schema evolution on live data is unsafe | Resolved (round 15) — committed baseline + db:push/db:deploy split + drift-guard spec |
| LOW | `bun audit` tail: 43 advisories (0 critical) in dev/CLI-time transitive trees | Dev-tooling supply-chain surface only (eslint/vitest chains, tailwind postcss, prisma CLI's effect); nothing ships to the served runtime | Accepted (round 15) — runtime set cleaned to 0 critical; wholesale `bun update` rejected as lockfile churn with zero runtime exposure |
...
| LOW | Inner gradient pane carries `transition-colors` only on dashboard/expenses | Live-side inconsistency — RETIRED by the live update (the live now carries the transition on every view; round 16 re-pinned the raw order) | Resolved (round 16) |
| MEDIUM | Scaffold dead tail: 31 vendored-but-unused shadcn primitives + 34 dead runtime dependencies shipped in every install | Dead weight in the served tree, larger supply-chain surface, misleading "available primitives" signal | Resolved (round 20) — the dead cluster deleted, the ui inventory + the extended dependency ban pinned by manifest-contracts specs (392 → 428 tests); the body DOM verified byte-identical pre/post |
| MEDIUM | Runtime DATABASE_URL resolution depended on the process CWD; `.env.example` documented a `src/lib/db-path.ts` contract that did not exist (phantom `db:seed` / `docs/DEPLOYMENT.md §4` references) | A relative `file:` URL could anchor differently for CLI vs bundled-server launch contexts; setup docs drifted | Resolved (round 21) — `src/lib/db-path.ts` (CLI-equivalent, CWD-independent anchoring + absolute/PG passthrough + crisp missing-env error) wired via `datasourceUrl`; `.env.example` aligned |
| — | Round-22 full re-probe: zero drift on every layer (body DOM, chrome, mobile nav, Tailwind v4 bundle + runtime cascade, HTTP/head, interactive sweep) | The fourth consecutive zero-drift round — verification only | Verified (round 22) — no code changes; gates green on the fresh environment (lint 0, tsc 0, 434/434, 67/67, build clean); `docs/plans/2026-09-22-parity-remediation-round22.md` |
| — | Round-23 full re-probe: zero drift on every layer (body DOM, chrome, mobile nav, Tailwind v4 bundle + runtime cascade, HTTP/head, focused interactive sweep) | The fifth consecutive zero-drift round — verification only | Verified (round 23) — no code changes; gates green (lint 0, tsc 0, 434/434, 67/67, build clean); the audit tail matching §10 exactly (43 advisories, ZERO runtime); `docs/plans/2026-09-22-parity-remediation-round23.md` |
| — | Round-24 full re-probe: zero drift on every layer (body DOM 10 surfaces, chrome, mobile nav end-to-end both sides on the production build, Tailwind v4 bundle + runtime cascade + consumer-color byte-match, HTTP/head incl. the SPA-200 quirk, focused interactive sweep) + the documentation-debt alignment | The sixth consecutive zero-drift round — verification + seven doc-drift fixes | Verified (round 24) — no code changes; the doc fixes: the push-runbook's wrong remote + stale no-CI claim, the 7→8 model count + wrong model names (README/SKILL/PAD), the PAD's stale 16.1.3/tailwind.config.ts tree entry/378 count/glossary quarterly wording, the SKILL Appendix B header; gates green (lint 0, tsc 0, 434/434, 67/67, build clean); the audit tail matching §10 exactly (43 advisories, ZERO runtime); `docs/plans/2026-09-23-parity-remediation-round24.md` |
| — | Round-25 full re-probe: zero drift on every layer (body DOM 10 surfaces, the raw-order chrome probes, mobile nav end-to-end both sides — 71/71 drawer nodes, Tailwind v4 bundle + runtime cascade + consumer colors, HTTP/head incl. the per-route description template verified in-browser, focused interactive sweep) | The seventh consecutive zero-drift round — verification only | Verified (round 25) — no code changes; the standard alignment set (this PAD v1.24 header, the AGENTS/CLAUDE/README/SKILL round-25 references, session_35, the refreshed screenshots); gates green (lint 0, tsc 0, 434/434, 67/67 in 3.3m, build clean); the audit tail matching §10 exactly (43 advisories, ZERO runtime); the seven round-24 doc fixes grep-verified holding; `docs/plans/2026-09-23-parity-remediation-round25.md` |
| — | Round-26 full re-probe: zero drift on every layer (body DOM 10 surfaces byte-identical to round-25's capture set, the raw-order chrome probes, mobile nav end-to-end both sides — 71/71 drawer nodes, Tailwind v4 bundle + runtime cascade + consumer colors, HTTP/head incl. the per-route description template + the root-route default re-verified in-browser, focused interactive sweep) | The eighth consecutive zero-drift round — verification only | Verified (round 26) — no code changes; the standard alignment set (this PAD v1.25 header, the AGENTS/CLAUDE/README/SKILL round-26 references, session_37, the refreshed screenshots); gates green (lint 0, tsc 0, 434/434, 67/67 in 3.4m, build clean); the audit tail matching §10 exactly (43 advisories, ZERO runtime); CI verified two ways (needles + GitHub run evidence) after a display-artifact false alarm resolved by hexdump; the round-24/25 doc fixes grep-verified holding; `docs/plans/2026-09-23-parity-remediation-round26.md` |
| — | Round-27 full re-probe: zero STRUCTURAL drift on every layer (body DOM 10 surfaces — 8 byte-identical to round-26's captures, Income carrying only the user's income-source deletion −27/+9 (the empty state now renders), the raw-order chrome probes, mobile nav end-to-end both sides — 71/71 drawer nodes, Tailwind v4 bundle + variable-level cascade + consumer colors, HTTP/head incl. the per-route description template verified in-browser + the root-route default re-verified, focused interactive sweep) | The ninth consecutive zero-drift round — verification only | Verified (round 27) — no code changes; the standard alignment set (this PAD v1.26 header, the AGENTS/CLAUDE/README/SKILL round-27 references, session_39, the refreshed screenshots); gates green (lint 0, tsc 0, 434/434, 67/67, build clean); the audit tail matching §10 exactly (43 advisories, ZERO runtime); CI verified two ways (marker-safe needles + od -c hexdump — the display artifact this round even ate `[m` through cat -A); the round-24/25/26 doc fixes grep-verified holding; one genuine doc-drift fix (the SKILL's stale 'has run 21 times' method count → 27); `docs/plans/2026-09-23-parity-remediation-round27.md` |

Verified in the 2026-09-22 full-stack verification round (round 22): no remediations — the live re-probe found zero drift on every probed layer for the fourth consecutive round (the body-DOM signature diff clean on all 10 surfaces, the desktop chrome and mobile drawer byte-identical with the drawer verified working end-to-end on both sides in dev AND production, the Tailwind v4 audit clean in bundle and runtime cascade — computed `#2563eb`/`#1e293b` on the CSS variables themselves — the HTTP layer byte-exact vs every prior pin, and the full interactive sweep matching: FAB, Quick Add both steps, Add Expense modal, AI Coach, Analytics chart configuration, theme toggle, a live expense CRUD round-trip), and the codebase production-readiness review found nothing to close (the gate chain green on a fresh environment, the audit tail matching §10 exactly, CI bytes hex-verified clean). Plan and verification record: `docs/plans/2026-09-22-parity-remediation-round22.md`.

Verified in the 2026-09-22 fifth zero-drift verification round (round 23): no remediations — the live re-probe found zero drift on every probed layer for the fifth consecutive round (the body-DOM signature diff clean on all 10 surfaces, the desktop chrome and mobile drawer byte-identical with the drawer verified working end-to-end on both sides in dev AND production, the Tailwind v4 audit clean in bundle and runtime cascade — computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables themselves — the HTTP layer byte-exact vs every prior pin, and the focused interactive sweep matching on both sides: FAB, chooser, step-2, the FAB close-toggle), and the codebase production-readiness review found nothing to close (the gate chain green, the audit tail matching §10 exactly — 43 advisories, ZERO in the runtime set — CI bytes marker-safe-verified clean, the live user's theme state left as found). Plan and verification record: `docs/plans/2026-09-22-parity-remediation-round23.md`.

Verified in the 2026-09-23 seventh zero-drift verification round (round 25): the live re-probe found zero drift on every probed layer for the seventh consecutive round (the body-DOM signature diff clean on all 10 surfaces with every delta in the documented buckets — the insight empty state, seed-data counts, chart SVG data, the lucide element-mix, the duplicated toaster, Next infra; the mobile drawer structurally byte-identical at 71/71 nodes and verified working end-to-end on both sides — open, navigate + unmount, Menu↔X both directions, X close, zero new console errors; the Tailwind v4 audit clean — the 176 KB bundle carries every drawer utility + keyframe with the reduced-motion gate, no dynamic class construction, the unlayered v3 palette pin winning the cascade (computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables) with consumer-element colors byte-matching the live; the raw-order chrome probes byte-identical — the aside/nav/innerPane strings and the active/inactive nav pills; the HTTP layer byte-exact vs every round-17→24 pin incl. the per-route description template verified in-browser on the live and the SPA-200 unknown-path quirk; the focused interactive sweep matching), and the codebase production-readiness review found the code side clean (no TODO/FIXME; the audit tail matching §10 exactly — 43 advisories, ZERO in the runtime set; CI bytes marker-safe-verified clean; the installed versions matching every doc claim; the seven round-24 doc fixes grep-verified holding) — a verification round, no code changes. Gates green on the fresh environment: lint 0, tsc 0, unit 434/434, E2E 67/67 in 3.3 minutes (after the chromium 1234 rebuild — the documented environment-lesson class), build clean; the live user's theme state left as found (dark, verified before and after). Plan and verification record: `docs/plans/2026-09-23-parity-remediation-round25.md`.
Verified in the 2026-09-23 eighth zero-drift verification round (round 26): the live re-probe found zero drift on every probed layer for the eighth consecutive round (the body-DOM signature diff clean on all 10 surfaces with every delta in the documented buckets — the insight empty state, seed-data counts, chart SVG data, the lucide element-mix, the duplicated toaster, Next infra — and the r26 capture set byte-identical to round-25's on every surface; the mobile drawer structurally byte-identical at 71/71 nodes and verified working end-to-end on both sides — open, navigate + unmount, Menu↔X both directions, X close, zero console messages on a clean cycle; the Tailwind v4 audit clean — the 176,431-byte bundle carries every drawer utility + keyframe with the reduced-motion gate, no dynamic class construction, the unlayered v3 palette pin winning the cascade (computed `#2563eb`/`#1e293b`/`#ef4444`/`#34d399`/`#4b5563` on the CSS variables) with consumer-element colors byte-matching the live; the raw-order chrome probes byte-identical — the aside/nav/innerPane strings and the active/inactive nav pills; the HTTP layer byte-exact vs every round-17→25 pin incl. the per-route description template verified in-browser on the live, the root-route default description re-verified, the login-only pins on the static login HTML, and the SPA-200 unknown-path quirk; the focused interactive sweep matching), and the codebase production-readiness review found the code side clean (no TODO/FIXME; the audit tail matching §10 exactly — 43 advisories, ZERO in the runtime set; CI verified two ways — the marker-safe needles plus the GitHub-side run evidence (12+ real runs on main), after a display-artifact false alarm resolved by hexdump: the tool-output rendering eats `[m` sequences in bracket-bearing lines, so verify such needles with od -c — a lesson extending the round-22/24 ANSI-artifact class; the installed versions matching every doc claim; the round-24/25 doc fixes grep-verified holding) — a verification round, no code changes. Gates green: lint 0, tsc 0, unit 434/434, E2E 67/67 in 3.4 minutes, build clean; the live account's theme state left as found (dark, verified before and after re-login — the mid-probe auth expiry showed the login page's light html, not a theme change). Plan and verification record: `docs/plans/2026-09-23-parity-remediation-round26.md`.
Verified in the 2026-09-23 ninth zero-drift verification round (round 27): the live re-probe found zero STRUCTURAL drift on every probed layer for the ninth consecutive round (the body-DOM signature diff clean on all 10 surfaces with every delta classified into the documented buckets, the single live change a user-data deletion — the last income source gone, the Income view rendering its empty state at −27/+9 while the other eight desktop surfaces and the mobile drawer stayed byte-identical to round-26's captures; the clone capture set byte-identical on all 9 surfaces), the mobile navigation menu verified working end-to-end on both sides against the production build (71/71 drawer nodes; open, navigate + unmount, Menu↔X both directions, X close, zero console messages), the TailwindCSS v4 audit clean (the 176,431-byte bundle with every drawer utility + the fin-* keyframes gated by prefers-reduced-motion:no-preference; the v3 palette pins verified at the variable level with the unlayered :root pin winning the runtime cascade — computed #2563eb/#1e293b/#ef4444/#34d399/#4b5563; no dynamic class construction; consumer colors byte-matching), the raw-order chrome probes byte-identical, the HTTP layer byte-exact against every round-17→26 pin (including the per-route description template verified in-browser on the live and the login-only pins, two of which render content-first attribute order with byte-matching values), and the focused interactive sweep matching on both sides — a verification round, no code changes: the 434 unit + 67 E2E executable contracts stand as the verification record, the deliverables are the round-27 alignment set (the plan, the refreshed screenshots, the AGENTS/CLAUDE/README/PAD/SKILL round-27 references, session_39), and one genuine documentation-drift fix shipped alongside (the SKILL's stale 'has run 21 times' re-probe-method count, stale since the round-21 distillation, corrected to 27).

Verified in the 2026-09-23 sixth zero-drift verification round (round 24): the live re-probe found zero drift on every probed layer for the sixth consecutive round (the body-DOM signature diff clean on all 10 surfaces with every delta in the documented buckets — the insight empty state, seed-data counts, chart SVG data, theme-state icons, the duplicated toaster, Next infra; the mobile drawer structurally byte-identical at 70/70 nodes and verified working end-to-end on both sides against the production build — open, navigate + unmount, Menu↔X both directions, zero new console errors; the Tailwind v4 audit clean — the 176 KB bundle carries every drawer utility + keyframe, no dynamic class construction, the unlayered v3 palette pin winning the cascade with consumer-element colors byte-matching the live; the HTTP layer byte-exact vs every round-17→23 pin incl. the SPA-200 unknown-path quirk; the focused interactive sweep matching), and the codebase production-readiness review's remediation surface was documentation debt: seven verified doc-drift fixes — the push-runbook's wrong remote (`task-management.git` → `financial-dashboard.git`) + its stale no-CI claim, the 7→8 Prisma model count + wrong model names (README/SKILL/PAD), this PAD's stale 16.1.3 Next.js version + the `tailwind.config.ts` directory-tree entry + the 378 CI test count + the glossary's monthly-equivalent quarterly wording, and the SKILL Appendix B header — each fix grep-verified against the measured codebase state (8 models in `prisma/schema.prisma`, next 16.3.5, 434 unit tests). No code changes; gates green; the audit tail matching §10 exactly (43 advisories, ZERO in the runtime set); CI bytes marker-safe-verified clean with the ANSI artifact re-demonstrated inside the round's own verification-script output labels; the live user's theme state left as found (dark). Plan and verification record: `docs/plans/2026-09-23-parity-remediation-round24.md`.
...
Resolved in the 2026-09-19 per-route description-template round (round 19): the standard full live re-probe (10-surface + login sorted + raw-order signature diffs, the per-element style/motion audit, the dialog/chooser/FAB/Progress/mobile structural probes, the VLM side-by-side sweep — 5/5 EQUIVALENT, the Analytics pair confirmed on a full-page re-comparison after an initial viewport-crop flag) found the live BODY DOM unchanged since round 18 — zero non-bucketed deltas on all 11 surfaces, RAW==SORTED everywhere (no class-order drift), every structural probe matching the round-14/16/18 pins (the motion audit's settled-element count on the live Dashboard moved 19 → 15, a DATA-bucket delta — the live user's recent-activity rows shrank to 2; the wrapper anatomy is unchanged). But the HTTP-layer audit found the live <head> moved a THIRD time in three rounds: the round-18 `{ViewLabel} on Finara. {SITE_DESCRIPTION.slice(0, 80)}.` description template (a 98-char mid-word truncation) now applies to the MAIN `description` and `og:description` metas on the eight non-dashboard view routes (probed on all 8; /, /Dashboard, /login and 404s keep the FULL text on all three description metas — round 18 had the template on twitter:description only), and /login renders `twitter:image:alt "Base44 link preview"` (the app pages keep the bare twitter:image; no twitter:image width/height anywhere — the og:image dims stay og-only). Fixes (TDD, 3 net-new specs): the seam's private `twitterDescription` generalized to `routeDescription` feeding `description` + `openGraph.description` + `twitter.description` (one per-route string everywhere), and the login twitter image carries the alt. Verified post-fix at the curl level (byte-exact template strings on /Goals and /Income; the login twitter:image:alt present; the round-17/18 pins — security headers, robots/sitemap, viewport split, icon pair, og:image shapes, twitter:url/card/title — re-verified unchanged) and the body-DOM signature diff is byte-identical pre/post (a transport-layer-only round). Plan, audit trail, and verification record: `docs/plans/2026-09-19-parity-remediation-round19.md`.

Resolved in the 2026-09-19 per-route head-structure round (round 18): the standard full live re-probe found the live BODY DOM unchanged since round 17 (zero non-bucketed deltas on all 10 surfaces in both sorted and raw-order diffs; the motion audit, the Add Expense modal, the FAB chooser, the Progress mechanism, the mobile drawer slide-in rAF sample, the SyncedBadge, the sidebar raw orders, and the user-menu anatomy all match the round-14/16 pins; VLM 4/5 EQUIVALENT with Goals flagging only the seed-data card count) — but the round-17 HTTP-layer audit, now part of the standard opener, found the live <head> RESTRUCTURED per route: the viewport/theme-color pair (viewport-fit=cover + theme-color #000000) moved to the LOGIN route only (app pages render the plain viewport), the icon links split (login: icon sizes="any" + apple-touch-icon sizes="180x180"; app pages: a single icon link — the live's type="image/svg+xml" mislabel on a PNG href deliberately not replicated), the og:image dims/alt became login-only (the live declares 1200×630 + alt "Base44 link preview" for its CDN render; the clone declares its own asset's honest 480×480 + the live's alt — asset values are deployment properties, structure is the contract), the twitter block follows a per-route template (the FULL description on /, /Dashboard, /login and 404s; `{ViewLabel} on Finara. {SITE_DESCRIPTION.slice(0, 80)}.` — a 98-char mid-word truncation — on the eight non-dashboard view routes, probed on all 8) with a BARE twitter:image and a twitter:url equal to the absolute canonical (rendered through `metadata.other` because Next's Twitter metadata type has no url field; the dashboard url is the bare origin, mirroring its canonical quirk), and NO keywords meta renders anywhere. Fixes (TDD, 11 net-new specs): `buildRouteViewport` in `src/lib/route-metadata.ts` + the catch-all page's `generateViewport` (login-only viewport fields, complete per route); the seam's login-level `icons` (page-level icons replace the layout's — the same shallow-per-key merge as openGraph) and og:image descriptor; the explicit per-route `twitter` block + `absoluteCanonicalUrl` (with `siteBaseUrl` trailing-slash normalization); the layout slim (plain viewport, icon-only, bare default og:image, keywords removed). The body-DOM regression check is byte-identical pre/post (179/179 both modes); the round-17 pins that hold (security headers, robots/sitemap, per-route og:title/og:url/canonical, the description, the PWA metas, the true-PNG favicon) were re-verified unchanged.

Resolved in the 2026-09-19 HTTP/metadata-layer round (round 17): the standard full live re-probe (9-view + login signature-multiset diff, BOTH sorted and the round-16 raw-order mode, the per-element style/motion audit, and the dialog/chooser/FAB/Progress/mobile structural probes) found the live app UNCHANGED since round 16 — zero non-bucketed deltas across all 10 surfaces (the only live-side change is the ACCOUNT DATA, reduced to a minimal set: 1 expense, 1 income source, 1 account, 1 goal, 1 budget — a data difference, not structure). The round therefore extended the parity audit to the transport/metadata layer the body-DOM diff is structurally blind to (curl-level probes), finding and fixing five gaps: (F1) the clone sent no security headers and leaked `X-Powered-By` — now the live's edge trio (Referrer-Policy strict-origin-when-cross-origin, X-Content-Type-Options nosniff, Strict-Transport-Security max-age=31536000) plus X-Frame-Options DENY + Permissions-Policy camera=(), microphone=(), geolocation=() apply to `/:path*` with `poweredByHeader: false`; (F2) the head metadata diverged — the layout now carries theme-color #000000, viewport-fit=cover, the live's exact description, the Finara logo as favicon/apple-touch-icon, and the apple-mobile-web-*/mobile-web-app-capable PWA metas, with per-route og:title/og:url via the new `buildRouteMetadata` pure seam in `src/lib/route-metadata.ts` (og:title mirrors the document title; the DASHBOARD canonical normalizes to `/` — a live quirk; the seam returns the COMPLETE openGraph block because Next merges metadata shallowly per key — a layout-level og block alone is silently REPLACED, caught in post-fix verification); (F3) the favicon was a generic emerald mark — now the Finara logo (converted from a mislabeled JPEG to a true PNG, same filename), with the dead `finara-icon.svg`/`logo.svg` scaffold assets removed; (F4) the per-route metadata logic lives in the pure seam (TDD-importable) with both page files as thin consumers; (F5) `public/robots.txt` (the old per-bot shape) is replaced by `src/app/robots.ts` rendering the live's wildcard shape + Sitemap line, and `src/app/sitemap.ts` serves the live's exact 9-URL set (`/` 1.0 + the eight non-dashboard views 0.8/weekly; no `/Dashboard`, no `/login` — /sitemap.xml previously fell through the catch-all to the NotFoundView HTML). Deliberate deltas: the live's `manifest.json` link 302s to nothing (broken base44 asset — not replicated); Next's auto-derived twitter:* metas (framework addition). `NEXT_PUBLIC_SITE_URL` (default `http://localhost:3000`) resolves metadataBase/og:url/sitemap URLs. Post-fix verification: the body-DOM signature diff is byte-identical pre/post (179/179 on the Dashboard — the round is transport-layer-only), and the HTTP layer matches the live's shapes at the curl level. Unit suite 359 → 378; E2E 67/67. Plan, audit trail, and verification record: `docs/plans/2026-09-19-parity-remediation-round17.md`.

Resolved in the 2026-09-19 raw-order + drawer-motion round (round 16): the re-probe found the LIVE UPDATED since round 15 — the SyncedBadge dropped its variant classes (rendering the Badge base only), every gradient pane now carries `transition-colors duration-300` (the round-4 dashboard/expenses-only quirk retired), and a NEW raw-order diff pass (tag|exact-class-attribute multisets, run alongside the long-standing sorted diff) exposed sidebar/shell class-ORDER deltas the sorted diff is structurally blind to — the lesson is now permanent: pair the sorted diff with a raw-order diff when auditing chrome, and re-extract RAW strings before pinning. Fixes, all live-probed and unit-pinned: the SyncedBadge renders via `variant="outline"` + the green tail (tw-merge strips `text-foreground`, producing the live string byte-exactly; the Badge PRIMITIVE is unchanged — its other badges still carry the default/secondary variant classes, verified); eight sidebar raw orders re-pinned (gradient container, header block, BOTH navs — the round-8 pin had passed via the drawer string while the desktop nav drifted — both bottom blocks, nav icons size-first, logo icon size-before-color, nav-pill className-before-tabIndex); the inner gradient pane re-ordered; MobileTopNav renders a FRAGMENT (top bar + drawer are direct shell children — the wrapper div removed); the Investments KPI cards motion-wrapped like the Holdings/Sector cards (a round-14 replication gap); and the mobile drawer mounts with the `fin-drawer-in` slide-in (translateX −300px→0 + opacity 0→1, ~330ms spring with +35px overshoot — rAF-sampled; `entranceStyle` gained the animationName parameter; the drawer stays conditionally rendered like the live's). The dialog-forms h-first ban now covers the two regex gaps the nav icons slipped through (dynamic member-expression tags + ternary className values). Dev-server screenshots of the remediated app added under `docs/screenshots/`; `.env.example` now documents `FINARA_INSIGHTS_LLM_OFF`. Unit 348 → 359; E2E 67/67. Plan, audit trail, and verification record: `docs/plans/2026-09-19-parity-remediation-round16.md`.

Resolved in the 2026-09-19 security + migration-baseline round (round 15): the standard live re-probe found ZERO drift (the signature diff semantically identical to the round-14 post-fix baseline; motion audit, dialog/chooser/FAB/Progress/Analytics/mobile probes all live-exact) — parity held, so the round closed the codebase's own production-readiness gaps. `bun audit` runtime criticals eliminated: next 16.1.3 → 16.3.5 (GHSA-2xp9-vwfh-vxw4 + GHSA-p293-qw3h-jr36 RCEs + GHSA-955p-x3mx-jcvp; the 16.2 security line was rejected because next@16.2.x's own optionalDependency pins `sharp ^0.34.5` — inside the vulnerable range — which would nest a second vulnerable sharp copy; 16.3.5 pairs with ^0.35.4 natively) with eslint-config-next locked to the same minor; sharp → 0.35.4 (GHSA-f88m-g3jw-g9cj); seven zero-import scaffold dependencies removed (next-auth critical GHSA-7rqj-j65f-68wh + high GHSA-xmf8-cvqr-rfgj, next-intl moderates, uuid moderate, @mdxeditor/editor + react-syntax-highlighter + @reactuses/core whose trees carried prismjs/js-cookie) — the manifest now matches the documented no-framer-motion policy. New unit contracts: `manifest-contracts.test.ts` (the advisory floors, the eslint lockstep, the banned set against package.json AND bun.lock) and `migrations-baseline.test.ts` (the drift guard). The §10 MEDIUM migration item resolved: committed Prisma baseline + `db:push` (dev/E2E) / `db:deploy` (production) split, verified baseline ≡ schema (migrate deploy → db push "already in sync"). Post-upgrade verification: all 9 view captures BYTE-IDENTICAL pre/post framework bump; audit tail 43 findings (0 critical), all dev/CLI-time — accepted. Unit 334 → 348; E2E 67/67 twice. Plan, audit trail, and verification record: `docs/plans/2026-09-19-parity-remediation-round15.md`.

Resolved in the 2026-09-19 motion-system round (round 14): the framer-motion entrance layer that round 13 misdiagnosed as removed (JS-driven animations never set CSS `animationName` — the round-13 probe checked CSS properties, so the inline-style animations were invisible) was re-found via a style-attribute audit + rAF animation sampling and CSS-replicated without the dependency (`entranceStyle`/`MotionWrap` + the `fin-card-in`/`fin-overlay-in`/`fin-scale-in` keyframes under `prefers-reduced-motion: no-preference`, per-view delay maps, capped row staggers); every dialog overlay re-ordered to the live string with settled `overlayStyle`s (only the Quick Add chooser animates open — fade + 0.9→1 scale); the login page's ten class orders re-pinned + Google label span-wrapped + h-first field icons (a new icon-order exception); the Expenses CardHeader wrapper removed (CardTitle + Tabs direct children); and the app-wide icon+text whitespace sweep (53 instances — the live emits no whitespace text nodes). Unit suite 311 → 334; E2E unchanged at 67. Plan, audit trail, and verification record: `docs/plans/2026-09-19-parity-remediation-round14.md`.

Resolved in the 2026-09-18 live-site-update round (round 13): the live app was updated after round 12, so a full 9-view re-probe (signature-multiset DOM diff, both directions) was run first — the AI Insights card was redesigned around persisted `TransactionInsight` records with a dismiss mechanism (the clone now has the `Insight` Prisma model, a pure draft builder in `src/lib/insights.ts`, a list-only GET that generates once when the table is empty behind an atomic marker, and `PATCH /api/ai/insights/[id]` for dismissal; the new card anatomy — type icon + raw-id badge, dismiss X, h4 title, description, blue suggested-action callout, Confidence/Category footer — is pinned by tests), entrance animations were removed app-wide, the full-modal dialog titles became DIVs, the Quick Add chooser was restructured to plain overlay > `w-full max-w-md` wrapper > plain card, the Quick Add Amount label gained its `space-y-2` field wrapper, and the Progress primitive switched to the live's indeterminate mechanism (manual transform, no aria-valuenow). The E2E gate surfaced two genuine defects, fixed TDD-first: the insights LLM polish broke gate hermeticity (nondeterministic text once collided with a KPI label assertion — now env-gated off in E2E) and its title-keyed join silently lost confidence/category whenever the LLM rewrote a title (now a positional join in `mergePolishedDrafts`). Unit suite 275 → 311; E2E 66 → 67 specs. Plan, audit trail, and verification record: `docs/plans/2026-09-18-parity-remediation-round13.md`.
...
| `src/components/finara/dashboard-view.tsx` | ~560 | KPI cards, action tiles, budget dot rows (indeterminate Progress), arrow-icon activity, AI-insight persisted records (round-13 anatomy: type badge, dismiss, suggested-action callout, confidence footer), quick actions, rotating FAB |
...
| `e2e/*.spec.ts` | 11 files / 67 specs | Golden-path lock: auth+titles, theme lifecycle, dashboard, expenses, quick-add, income, goals, accounts+investments, analytics, import-export, settings+theme, mobile+console (round 12) |
...
| `src/lib/db-path.ts` | ~85 | Server-only DATABASE_URL resolver (round 21): relative `file:` URLs anchor at `prisma/schema.prisma` (CLI-equivalent, CWD-independent walk-up), absolute/PostgreSQL URLs pass through, missing env throws the setup error; wired into `db.ts` via `datasourceUrl`; pinned by `src/lib/__tests__/db-path.test.ts` |
| `src/components/finara/ui-bits.tsx` | ~330 | Source-exact shared composites (StatCard, GradientCard, SectionCard, ViewHeader, …) + the round-14 entrance-motion helpers (`entranceStyle`, `MotionWrap`) |
...
| `src/components/finara/ai-coach-dialog.tsx` | ~146 | Centered chat modal (h-[80vh]) with quick actions + grounded replies; round-13: div title via asChild |
| `src/lib/insights.ts` | ~135 | Round-13 pure seam: deterministic insight drafts (≤3, live record fields) + `mergePolishedDrafts` (positional LLM-polish join) |
| `src/app/api/ai/insights/route.ts` | ~180 | Round-13: pure list (newest-10, dismissed filtered) + generate-once-when-empty behind the atomic `_insights_generated` marker; env-gated LLM polish |
| `src/app/api/ai/insights/[id]/route.ts` | ~30 | Round-13: `PATCH {is_dismissed:true}` dismissal (idempotent) |
| `src/lib/types.ts` | ~165 | DTO contract for the whole client/server boundary (round-13: `AiInsightDto` = the live record field set) |
| `src/app/api/import/route.ts` | ~119 | Batch import (CSV rows + finara-export mode) with row-level validation |
| `prisma/schema.prisma` | ~115 | 8 models (round-13: + `Insight`), integer minor-unit money |
| `src/lib/ui-maps.ts` | ~115 | Live-verified visual maps: sector hexes, goal emoji, priority/activity/expense-row badges, icons + the round-13 insight type icon/badge maps (ADR-011) |
...
| `src/lib/route-metadata.ts` | ~140 | Round-17/18/19 pure seam: `buildRouteMetadata` (per-route title + canonical + the COMPLETE openGraph — Next merges metadata shallowly; round 18 adds the explicit twitter block, `twitter:url` via `other`, and the login-only icons/og-image descriptor; round 19's `routeDescription` feeds all three description metas and the login twitter image carries the alt) + `buildRouteViewport` (login-only viewport-fit/theme-color) + `absoluteCanonicalUrl`/`siteBaseUrl`/`SITE_DESCRIPTION` |
| `src/app/robots.ts` / `src/app/sitemap.ts` | ~30 / ~38 | Round-17 SEO files: the live's wildcard robots shape + Sitemap line; the 9-URL sitemap (`/` 1.0 + eight non-dashboard views 0.8/weekly) |
| `src/lib/__tests__/site-metadata.test.ts` | ~330 | Round-17/18/19 HTTP/metadata contracts: the security-header baseline, the layout head pins (round-18 re-pins: the plain viewport, icon-only, no keywords), the route-metadata seam (og/twitter tables, the per-route description template — round 19 extends it to description + og:description, `twitter:url`, the login twitter-image alt), the viewport split, the robots/sitemap shapes, the PNG favicon magic bytes |
| `src/lib/__tests__/manifest-contracts.test.ts` | ~250 | Round-15 dependency floors + banned set; round-20: the extended 34-package ban (package.json AND bun.lock) + the ui-inventory contract (`src/components/ui/` = exactly the 17 app-rendered primitives, both directions) |
| `src/lib/demo-user.ts` | ~25 | Demo account identity for the GDPR export envelope/filename (ADR-022) |
| `src/lib/__tests__/functional-parity.test.ts` | ~660 | Round-9 functional source contracts (ADR-022) + round-10 theme-lifecycle contracts (ADR-023) + round-11 interactive-state contracts (ADR-024) + round-12 title/import/FAB contracts + round-13 insight/dialog/progress/animation contracts |
...
| **Monthly equivalent** | An income amount normalized to its monthly value (biweekly ×26/12, weekly ×52/12, annual ÷12; `quarterly` ÷3 and `one-time` → 0 are legacy-data guards only — ADR-007) |
