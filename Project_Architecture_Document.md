...
| Unit suite | 348 tests / 16 files | Vitest 5 (node env) | `src/lib/__tests__/` |
...
Vitest covers the pure domain layer (`src/lib` behavior modules + the primitive/source-contract pins) with 311 tests; thresholds are not enforced numerically yet — the rule is "every behavior change lands with its failing test first" (ADR-009). The Playwright E2E layer over the golden paths (67 specs, §7.1) pins the AI-INSIGHT record lifecycle too (generation-on-empty, dismiss, refresh re-list — deterministic under `FINARA_INSIGHTS_LLM_OFF=1`); the AI chat surface stays unit-pinned only (external model dependency).

### 7.4 Pre-Push Checklist

- [ ] `bun run lint` exits 0
- [ ] `bun run typecheck` exits 0
- [ ] `bun run test` — 311 tests, 0 failures
...
| `src/lib/route-metadata.ts` | ~55 | Round-17 pure seam: `buildRouteMetadata` (per-route title + canonical + the COMPLETE openGraph — Next merges metadata shallowly) + `siteBaseUrl`/`SITE_DESCRIPTION` |
...
| `src/lib/__tests__/site-metadata.test.ts` | ~230 | Round-17 HTTP/metadata contracts: the security-header baseline, the layout head pins, the route-metadata seam, the robots/sitemap shapes, the PNG favicon magic bytes |
