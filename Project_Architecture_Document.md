**Last Updated:** 2026-09-19 (v1.19 — the dead-code & dependency-hygiene round 20: the standard full live re-probe (10-surface + login sorted + raw-order signature diffs, motion audit, dialog/chooser/FAB/Progress/mobile structural probes, VLM sweep, HTTP-layer audit) found the live UNCHANGED on EVERY layer — the first zero-drift head round since 16: the body DOM is byte-identical to the r19 captures on 10 of 11 surfaces (Analytics differs only by the live user's shrinking trend-line dot count — a pure data-bucket delta), RAW==SORTED everywhere, every structural probe matches its pin (the motion audit settles at 19 again — the live user's data grew back), and the HTTP layer matches every round-17/18/19 pin byte-exact; the VLM sweep returned 5/5 EQUIVALENT (Expenses + Dashboard initially flagged on full-page scaling artifacts — the clone's 96-row Expenses page is far taller so the scaled-down comparison reads as a "narrower sidebar"; the viewport-only re-comparisons returned EQUIVALENT and the measured sidebar is exactly 256px on both sides). Zero parity drift, so the round closed the codebase's own production-readiness gaps, round-15 style: the base44 scaffold's 31 vendored-but-unused shadcn primitives deleted (a closed cluster — zero imports from app code, the pinned set, or any spec; `src/components/ui/` ends at exactly the 17 app-rendered files, pinned both directions by the new ui-inventory contract) and 34 dead runtime dependencies removed (9 zero-import — including the look-alikes @tanstack/react-query, date-fns, zod, zustand — plus 25 imported only by the dead files: 15 unrendered Radix packages, cmdk, embla-carousel-react, input-otp, next-themes, react-day-picker, react-hook-form, react-resizable-panels, sonner, vaul; the runtime set is 24 packages, every one imported by app code; even @radix-ui/react-separator fully left the tree — the kept dropdown-menu renders its separator through the dropdown-menu package itself). TDD: the manifest-contracts RED specs (34 package.json exclusions + the lockfile-absence table + the ui inventory) failed 36/428 exactly as designed, then went green on the removal. The body-DOM regression check is byte-identical pre/post (525/525 on the Dashboard root, 268/268 on Goals, both modes — a pure infra round); the HTTP-layer pins re-verified on the fresh build; `bun audit` unchanged (the tail is the accepted dev/CLI tooling set — the removed packages carried no runtime advisories). Doc-drift fixes: the AGENTS ADR-015 pinned-set enumeration corrected (dialog in — the suite pins it; separator out — never app-rendered), the README component/test counts aligned (17 components + theme store; 428 tests). Unit suite 392 → 428; E2E stays 67/67. Previously v1.18 — the per-route description-template round 19: the standard full live re-probe (10-surface + login sorted + raw-order signature diffs, motion audit, dialog/chooser/FAB/Progress/mobile structural probes, VLM sweep) found the live BODY DOM unchanged since round 18 — zero non-bucketed deltas on all 11 surfaces, RAW==SORTED everywhere — but the HTTP-layer audit found the live <head> moved a THIRD time: the round-18 `{ViewLabel} on Finara. {SITE_DESCRIPTION.slice(0, 80)}.` template now applies to the MAIN `description` and `og:description` metas on the eight non-dashboard view routes (round 18 had it on twitter:description only — the seam's `routeDescription` now feeds all three description fields, one per-route string everywhere, byte-verified on all 8), and /login renders `twitter:image:alt "Base44 link preview"` (app pages keep the bare twitter:image; no twitter:image width/height on any route — the og dims stay og-only). The body-DOM regression check is byte-identical pre/post (525/525 on the Dashboard, 268/268 on Goals — a transport-layer-only round); unit suite 389 → 392; E2E stays 67/67. Previously v1.17 — the per-route head-structure round 18: the standard full live re-probe (sorted + raw-order signature diffs, motion audit, dialog/chooser/FAB/Progress/mobile structural probes, VLM sweep) found the live BODY DOM unchanged since round 17 — zero non-bucketed deltas on all 10 surfaces — but the HTTP-layer audit found the live <head> RESTRUCTURED per route: the viewport/theme-color pair, the apple-touch-icon (+ the login icon sizes), and the og:image dims/alt moved to the LOGIN route only, the twitter block gained a per-route description template plus twitter:url, and no keywords meta renders anywhere; fixes — `buildRouteViewport` + the catch-all `generateViewport`, the seam's login-level icons + og:image descriptor, the explicit per-route twitter block with `twitter:url` via `metadata.other` (Next's Twitter type has no url field) + `absoluteCanonicalUrl`, the layout slim; unit suite 378 → 389. Previously v1.16 — the HTTP/metadata-layer round 17: the standard full live re-probe (sorted + raw-order signature diffs, motion audit, dialog/chooser/FAB/Progress/mobile structural probes) found the live app UNCHANGED since round 16 — zero non-bucketed deltas on all 10 surfaces; the only live-side change is the ACCOUNT DATA (reduced to a minimal set — 1 expense, 1 income source, 1 account, 1 goal, 1 budget — a data difference, not structure). The round extended the parity audit to the transport/metadata layer the body-DOM diff is structurally blind to (curl-level: response headers, <head> metadata, robots.txt, sitemap.xml, favicon) and closed five gaps: the security-header baseline (the live's edge trio — Referrer-Policy strict-origin-when-cross-origin, X-Content-Type-Options nosniff, Strict-Transport-Security max-age=31536000 — plus X-Frame-Options DENY + Permissions-Policy, with poweredByHeader: false); the head metadata (theme-color #000000, viewport-fit=cover, the live's exact description, the Finara logo as favicon/apple-touch-icon, the apple/PWA metas, and per-route og:title/og:url via the new `buildRouteMetadata` pure seam — og:title mirrors the document title, the DASHBOARD canonical normalizes to `/` (a live quirk), and the seam returns the COMPLETE openGraph block because Next merges metadata SHALLOWLY per key — a layout-only og block is silently replaced); the favicon identity (the generic emerald mark replaced by the Finara logo, itself converted from a mislabeled JPEG to a true PNG; dead scaffold assets removed); and the SEO files (`app/robots.ts` renders the live's wildcard robots shape + Sitemap line — the old per-bot `public/robots.txt` deleted — and `app/sitemap.ts` serves the live's exact 9-URL set: `/` 1.0 + the eight non-dashboard views 0.8/weekly, no `/Dashboard`, no `/login`). `NEXT_PUBLIC_SITE_URL` (default `http://localhost:3000`) resolves metadataBase/og:url/sitemap URLs. Post-fix: the body-DOM signature diff is byte-identical pre/post (179/179 on the Dashboard — a transport-layer-only round) and the HTTP layer matches the live's shapes at the curl level. Deliberate deltas: the live's broken manifest.json link not replicated; Next's auto-derived twitter:* metas documented as a framework addition. Unit suite 359 → 378 (the new site-metadata contracts); E2E stays 67/67. Prior: v1.15 —  — raw-order + drawer-motion round 16: the re-probe found the LIVE UPDATED since round 15 — the SyncedBadge dropped its variant classes, every gradient pane now carries `transition-colors duration-300`, and a NEW raw-order diff pass (tag|exact-class-attribute multisets, alongside the long-standing sorted diff) exposed a set of sidebar/shell class-ORDER deltas the sorted diff is structurally blind to. Fixes (all live-probed, all unit-pinned): the SyncedBadge renders the Badge base only (`variant="outline"` + tail — tw-merge strips `text-foreground`, so no variant classes survive; the Badge PRIMITIVE is unchanged, verified via its other badges); eight sidebar raw orders re-pinned (gradient container `flex flex-col flex-1 min-h-0 sidebar-gradient`, header block `flex items-center h-16 px-6 border-b …`, BOTH navs `flex-1 px-4 py-6 space-y-2` — the round-8 pin had passed via the drawer string while the desktop nav drifted, both bottom blocks `p-4 …` first, nav icons size-first `w-5 h-5`/`w-6 h-6` — an ADR-020 violation that slipped the dialog-forms ban through the dynamic `item.icon` ternary, now covered — logo icon size-before-color, nav pill className-before-tabIndex); the inner gradient pane re-ordered to the live string; MobileTopNav renders a FRAGMENT (top bar + drawer are direct shell children — no wrapper div); the Investments KPI cards motion-wrapped like the Holdings/Sector cards; and the mobile drawer mounts with the `fin-drawer-in` slide-in (translateX −300px→0 + fade, ~330ms spring, +35px overshoot — rAF-sampled; `entranceStyle` gained the animationName parameter). The §10 pane-quirk row is retired. Unit suite 348 → 359; E2E stays 67/67. Screenshots of the remediated app now live in `docs/screenshots/`. Prior: v1.14 — security + migration-baseline round 15: the standard full live re-probe (9-view + login signature-multiset diff, per-element style-attribute/motion audit, dialog/chooser/FAB/Progress/Analytics/mobile probes) found the live app UNCHANGED since round 14 — the diff was semantically identical to the round-14 post-fix baseline and every structural probe matched the pins, so visual parity held with nothing to remediate. The round pivoted to the codebase's own production-readiness gaps: `bun audit` showed 90 advisories with 3 criticals in the runtime set — next 16.1.3 (GHSA-2xp9-vwfh-vxw4 + GHSA-p293-qw3h-jr36 unauthenticated RCEs, GHSA-955p-x3mx-jcvp disclosure; fixed 16.2.5+) and next-auth 4.24.11 (GHSA-7rqj-j65f-68wh critical) — plus a high in sharp (GHSA-f88m-g3jw-g9cj libvips CVEs, fixed 0.35.0+) and four more unused advisory-carrying dependencies. Remediation: next → 16.3.5 (the 16.2 line was rejected — its own optionalDependency pins sharp ^0.34.5, inside the vulnerable range, which would nest a second vulnerable copy; 16.3.5 pairs with ^0.35.4 natively) with eslint-config-next locked to the same minor; sharp → 0.35.4; SEVEN zero-import scaffold dependencies removed (next-auth, next-intl, framer-motion, uuid, @mdxeditor/editor, react-syntax-highlighter, @reactuses/core — the manifest now matches the documented no-framer-motion policy); the floors + lockstep + removals are pinned by manifest-contracts specs against package.json AND bun.lock. The PAD §10 MEDIUM migration-history item was closed: a committed Prisma baseline (`prisma/migrations/20260919000000_init/migration.sql` via `prisma migrate diff --from-empty` + migration_lock.toml) with the workflow split — dev/E2E keep `db:push`, production deploys use the new `db:deploy` (`prisma migrate deploy`); a migrations-baseline drift-guard spec fails the unit gate if a schema model lacks its CREATE TABLE. Verified: migrate deploy → db push reports "already in sync" (baseline ≡ schema); the post-upgrade 9-view DOM captures are BYTE-IDENTICAL to pre-upgrade (zero rendering delta from the framework bump); audit tail after remediation 43 findings (0 critical), all dev/CLI-time transitive trees (eslint/vitest tooling, tailwind postcss, prisma CLI's effect) — accepted and recorded. Unit suite 334 → 348; E2E stays 67/67 (run green twice — after the bump and after the removals). Prior: v1.13 — motion-system round 14: a full re-probe (9-view signature diff + per-element style-attribute audit + rAF/MutationObserver animation sampling) found the live app still runs the framer-motion entrance layer that round 13 misdiagnosed as "removed" (JS-driven animations never set CSS `animationName` — only the `.fade-in-up`/`.stagger-N` CSS classes were gone). The clone now CSS-replicates the system without the dependency: `entranceStyle(delayMs)` + `MotionWrap` in `ui-bits.tsx` render the live's settled inline style (`opacity: 1; transform: none`) plus an inline animation referencing the `fin-card-in` keyframes (hand-fit to the sampled spring: opacity 0→1 + translateY 20px→0, ~330ms, −2.3px overshoot, ~100ms stagger; defined ONLY under `prefers-reduced-motion: no-preference` so reduced-motion users render settled); per-view delay maps follow the measured stagger (Dashboard's feature grid animates before the tiles) and list-row staggers are CAPPED (`min(i, 8)×50ms` — the live's uncapped stagger would take 9.8s on the 98-row seed); the Quick Add chooser ANIMATES open (overlay `fin-overlay-in` fade + wrapper `fin-scale-in` 0.9→1 spring) while the full modals open instantly with settled `overlayStyle`s (`{opacity: 1, transform: "none"}` — Add Account `{opacity: 1}` with its own overlay order); the dialog overlay base was re-ordered to the live string; the login page's ten class orders were re-pinned + the Google label span-wrapped + field icons switched h-first; the Expenses CardHeader renders CardTitle + Tabs as direct children; and all 53 icon+text pairs now render space-free (the live emits no whitespace text nodes — multi-line JSX idiom, sweep-pinned). Unit suite 311 → 334; E2E stays 67/67. Prior: v1.12 — live-site-update round 13: the live app was updated after round 12 and the clone re-probed (9-view signature-multiset DOM diff) and re-aligned. AI Insights became PERSISTED `Insight` records (new Prisma model; the GET lists newest-10 non-dismissed, dismisses via `PATCH /api/ai/insights/[id] {is_dismissed:true}`, and generates deterministic drafts ONCE when the table is empty — an atomic `_insights_generated` Setting marker makes the generation right exclusive across React StrictMode double-fetches; the optional LLM polish joins polished text onto drafts POSITIONALLY via `mergePolishedDrafts` and is env-gated (`FINARA_INSIGHTS_LLM_OFF=1` in the E2E webServer — the polish once collided nondeterministically with a KPI label assertion; record text keeps its creation-time currency like the live's externally-generated rows); the card anatomy renders type icon + raw-id badge + dismiss X + h4 title + description + blue suggested-action callout + Confidence/Category footer, with blank-while-loading and the brain empty state), entrance animations REMOVED app-wide (the live dropped every `.fade-in-up`/`.stagger-N`), full-modal dialog titles became DIVs (`DialogTitle asChild`, aria-labelledby kept), the Quick Add chooser restructured to plain overlay > `w-full max-w-md` wrapper > plain card, the Quick Add Amount label wrapped in a `space-y-2` field wrapper, and the Progress primitive now renders the live's `data-state=indeterminate` + manual `translateX(-{100-value}%)` with NO aria-valuenow (Dashboard budget + Goals); 311-test unit suite + 67-spec E2E suite. Prior: v1.11 — quality-infrastructure round 12: the Playwright E2E layer (66 specs / 11 files in `e2e/`, running against the production build on :3100 with a hermetic `db/e2e.db` — locking every browser golden path from rounds 4–11: login round trips + titles, theme lifecycle, expense CRUD + filters + bulk bar, FAB Quick Add + the FAB-as-close-toggle contract, income/goal/account/investment CRUD incl. the complete-state ring, analytics tabs + ghost tooltip + inert From/To, CSV import + exports verified **settings-independent** under EUR + dd/MM/yyyy (live probe: ISO dates + raw decimals under both settings — byte-identical), GDPR round-trip, EUR/dd-MM/yyyy propagation, mobile drawer, and a zero-console-errors sweep over all 9 views), the GitHub Actions CI pipeline (`.github/workflows/ci.yml` — lint → typecheck → unit → build + E2E on every push/PR to main), three live-probed parity fixes surfaced by the E2E work (the login-surface title contract — bare `Finara` on /login AND on unauth deep-link redirects, with an unauth deep link settling on it via a render-time route adjustment that reads the LIVE session store; the `<title>`-hydration mechanism — the App Router rewrites the title node's textContent AFTER client effects, so the title sync is a MutationObserver-guarded post-commit effect; the import path now accepts negative amounts like every other layer — ADR-024 alignment for rows mode + finara-export restore; the Quick Add chooser is `modal={false}` with `onInteractOutside` suppressed — Radix's modal body-lock `pointer-events:none` deadened the z-50 FAB toggle), and `tailwind.config.ts` + the dead `tailwindcss-animate` dependency deleted (v4 CSS-first pipeline, `tw-animate-css` via CSS import); 275-test unit suite + 66-spec E2E suite. Prior: v1.10 — round 11: ADR-024 signed minor units (the live app performs ZERO amount validation — negative expenses/income/balances/goals/investments persist and render `-$5.50` style, so every `min="0"` was removed from the forms, `toMinorUnits` accepts negatives, and the API guards became `requireSignedInt`/`requireFiniteNumber`; negative goal contributions stay a silent no-op and progress is never capped — 150% over target renders 150.0%), the goal complete-state contract (emerald card ring + Complete badge with the circle-check-big icon + the Add Progress button removed entirely at progress ≥ 100%), the chart tooltip = the live's borderless ghost overlay (its raw-HSL-triple tokens make the inline `var()` color refs invalid-as-color — background transparent, border shorthand fully invalidated, inherited text; replicated as `{ backgroundColor: "transparent", border: "none", borderRadius: 8 }` and verified by computed-style byte-match + tooltip-crop pixel analysis in both themes), legend census 1/0/0/0, the null default expense-filter min (negatives visible), investment-form attrs aligned (steps 0.01/0.1, optional current price → 0), Select-popup class pins, and `formatMoney(-0)` → `-$0.00`; 270-test suite. Prior: v1.9 — round 10: ADR-019 primary-family amendment (`--primary`/`--primary-foreground` re-pinned to the live neutral ink/paper pair via a variable-level probe — light `#171717`/`#fafafa`, dark `#fafafa`/`#171717`; dark `--ring #d4d4d4`; dark `--destructive #7f1d1d`; `--destructive-foreground #fafafa` both themes — progress fills, switches, checkboxes, and default buttons render monochrome; sage CTAs keep `--primary-sage`), ADR-023 theme lifecycle (sign-out resets to light + clears the stored key; sign-in re-applies the server-side theme from `/api/settings` async like the live User/me hydration; every toggle persists fire-and-forget — the theme follows the account across sign-out/sign-in cycles), mobile menu button swaps Menu↔X while the drawer is open, first full light+dark VLM screenshot sweep since round 5 (16/18 pairs visually equivalent pre-fix; the 3 flagged pairs re-shot post-fix: all equivalent; EUR quick-amount chips, Save-Settings flow, and the live-shared overflow quirks verified), 251-test suite
...
| Unit suite | 428 tests / 17 files | Vitest 5 (node env) | `src/lib/__tests__/` |
...
Vitest covers the pure domain layer (`src/lib` behavior modules + the primitive/source-contract pins) with 392 tests; thresholds are not enforced numerically yet — the rule is "every behavior change lands with its failing test first" (ADR-009). The Playwright E2E layer over the golden paths (67 specs, §7.1) pins the AI-INSIGHT record lifecycle too (generation-on-empty, dismiss, refresh re-list — deterministic under `FINARA_INSIGHTS_LLM_OFF=1`); the AI chat surface stays unit-pinned only (external model dependency).

### 7.4 Pre-Push Checklist

- [ ] `bun run lint` exits 0
- [ ] `bun run typecheck` exits 0
- [ ] `bun run test` — 392 tests, 0 failures
