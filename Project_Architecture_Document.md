...
| Unit suite | 251 tests / 13 files | Vitest 5 (node env) | `src/lib/__tests__/` |
...
| `src/components/finara/finara-app.tsx` | ~215 | SPA shell: session store, view switching, dialog wiring, per-view gradient panes, FAB toggle, sign-out theme reset + sign-in server-theme apply (ADR-023) |
...
| `src/lib/__tests__/functional-parity.test.ts` | ~420 | Round-9 functional source contracts (ADR-022) + round-10 theme-lifecycle contracts (ADR-023) |
