...
Entrance animations were REMOVED app-wide in round 13 (live-probed: the app updated after round 12 — zero `.fade-in-up`/`.stagger-N` elements, zero CSS rules, `animationName: none`; ADR-013 is superseded for entrances). The one remaining entrance is the AI-insight row (`insight-row` fade + slide-left, 0.1s stagger via `animation-delay`, `prefers-reduced-motion` gated — the CSS replica of the live's framer-motion entrance; the exit animation is a documented delta). Card hover keeps `.card-hover` (translate + shadow lift); dialog scale/fade stays via `tw-animate-css`; chart mount animations from Recharts defaults.
...
| Unit suite | 311 tests / 14 files | Vitest 5 (node env) | `src/lib/__tests__/` |
