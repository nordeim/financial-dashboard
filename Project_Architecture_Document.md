**Last Updated:** 2026-09-17 (v1.7 — order-parity + real-route remediation round 8: ADR-021, live real-route architecture (/Dashboard…/Settings via the `[...view]` catch-all + `src/lib/routes.ts`, `/` = dashboard with no active pill, per-route titles, standalone 404 page, `/login?from_url=` SPA redirect flow, pushState/popstate client navigation), rendered-class-ORDER parity sweep across all nine views (172 order deltas to 0; ui-bits CARD_DASHBOARD_SURFACE/CARD_HOVER/CARD_PLAIN constants replace CARD_SURFACE), Button default-variant `shadow` pin, ClassicTrash2 single-alias icon, expenses pagination removal + live blue bulk-bar anatomy (Select All/Deselect All w/ SquareCheckBig, always-plural count, inert Bulk Edit, red Delete, X clear), Quick Add step-2 flex-1 buttons + Check glyph + disabled-until-valid, mobile header/drawer re-pins, SyncedBadge via Badge, six views' space-y-8 root wrappers removed, 210-test suite
