...
- The runtime resolves `DATABASE_URL` through `src/lib/db-path.ts` (round 21): relative `file:` URLs anchor at `prisma/schema.prisma` (CLI-equivalent, CWD-independent), absolute URLs (the E2E `db/e2e.db`) pass through, and a missing value throws the crisp setup error. `db-path` is server-only — never import it from client code.
...
| `bun run test` / `test:watch` | Vitest unit suite (434 tests) / watch mode |
...
- **Pre-push gate**: `bun run lint && bun run typecheck && bun run test` (434 Vitest specs) + `bun run test:e2e` (67 Playwright specs); CI (`.github/workflows/ci.yml`) runs the same chain on every push/PR to main.
