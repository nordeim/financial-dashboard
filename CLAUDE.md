...
1. **ANALYZE** — Read the relevant view component, its API route, and `src/lib/{types,categories,money,ui-maps}.ts` in full before writing. Identify which surface owns the data you are changing. For parity work, re-read the round-6 plan `docs/plans/2026-09-17-parity-remediation-round6.md` (token pins, per-dialog matrix, overlay behavior) and round-5 §A/§E.9 (and round-4 §A/§E, round-3 § audit findings, round-2 § "Key live-site behavioral facts") first — class-exact restyles copy the strings from the captured live DOM evidence, never approximations.
...
5. **VERIFY** — `bun run lint && bun run typecheck && bun run test` green (122 unit tests); `bun run build` exits 0; exercise the flow in a browser (agent-browser or manual) and check the console for zero errors. Claims of "works" require executed evidence — label anything not executed as Reasoned/Assumed. Parity changes additionally require computed-style probes (font stack, semantic tokens, radii, blur, chart colors in both themes) and a signature-multiset DOM re-diff against the captured live app, with every only-live/only-clone delta accounted for in the documented buckets (infra, a11y additions, lucide artifacts, live's duplicated toaster, seed-data counts, the login demo-credentials affordance).
...
| `bun run test` / `test:watch` | Vitest unit suite (122 tests) / watch mode |
| `bun run db:push` / `db:generate` | Apply schema / regen client |

## Testing Strategy

- **Pre-push gate**: `bun run lint && bun run typecheck && bun run test` (122 Vitest specs) + `bun run build`; browser-verify the touched flow and confirm zero console errors.
