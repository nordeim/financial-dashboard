...
- `bun run test` → 348 tests passing (Vitest).
...
- **Dependency posture (round 15):** zero critical advisories in the runtime dependency set — Next.js 16.3.5 (above the 16.2.5 RCE advisory floor) with `eslint-config-next` locked to the same minor, `sharp` 0.35.4 (above the libvips floor), and seven zero-import scaffold dependencies removed (next-auth, next-intl, framer-motion, uuid, @mdxeditor/editor, react-syntax-highlighter, @reactuses/core — several carried their own advisories). The floors and the removal are pinned by `src/lib/__tests__/manifest-contracts.test.ts`; the remaining `bun audit` tail is dev/CLI-tooling only (eslint/vitest chains, tailwind's postcss, the prisma CLI's effect) and never ships to the served runtime.
